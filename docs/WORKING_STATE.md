...
- The managed browser automation path is Windows-only for v1, expects `BROWSER_AUTOMATION_ENABLED=true`, and needs a persistent Chrome profile via `BROWSER_USER_DATA_DIR` plus optional `BROWSER_CHROME_EXECUTABLE`. It does not attach to arbitrary already-open tabs or windows outside the managed session.
