...
- The managed browser automation path now has a first Gmail/email layer on top of the same Playwright Chrome session. Registered tools are `gmail_open`, `gmail_search`, and `gmail_compose_draft`. Gmail open/search are low-risk read/navigation actions; composing a draft is medium-risk and waits for approval before touching Gmail. Automated Gmail send is intentionally not implemented yet.
...
- Generic browser tasks now have a bounded multi-step agent loop. The coordinator can continue up to eight browser-agent tool calls, feeding the latest browser page state back into model planning after each read-only action. The terminal condition is still a `notify_user` call. Gmail/email requests route to the site-specific Gmail tools when possible rather than relying only on generic target IDs.
...
- The managed browser automation path is Windows-only for v1, expects `BROWSER_AUTOMATION_ENABLED=true`, and needs a persistent Chrome profile via `BROWSER_USER_DATA_DIR` plus optional `BROWSER_CHROME_EXECUTABLE`. It does not attach to arbitrary already-open tabs or windows outside the managed session. Gmail automation assumes that this managed profile is already signed in; if it lands on Google sign-in, the tool reports that authentication is blocked rather than entering credentials.
...
1. Live-test `gmail_open`, `gmail_search`, and `gmail_compose_draft` on Windows with a dedicated signed-in Chrome profile; tune selectors against real Gmail DOM changes.
2. Decide the approval design for irreversible external email sends before adding any automated Gmail send tool.
3. Verify `screen_capture` from a signed-in interactive Windows session and decide whether the next agent step should consume the saved file path directly or attach richer image metadata.
...
- Gmail automation is DOM-selector based and has only mock/unit validation in this environment. Real Gmail may require selector tuning, and account login, MFA, CAPTCHA, and send-confirmation flows are deliberately out of scope for this slice.
